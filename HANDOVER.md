...
## 2026-07-06 追記: usage-summary API / 開封率についての重要な注意

サイトマイスター本体側のLINEエージェント（御社→クライアント向け、更新依頼・提案担当）が
LINE MAエージェントの利用状況を読みに行けるよう `GET /api/tenants/:tenantId/usage-summary` を追加。
未導入クライアントには「導入イメージ」、導入済みには「実績」を提案文に織り込むための想定。

**要注意**: LINE Messaging APIのpush配信には開封率(既読率)を取得する仕組みが存在しない。
フロントエンドのデモ（配信履歴・ダッシュボード）にある「開封率」はモックのダミー値。
`usage-summary` は実際に取得可能な指標（友だち数・連携率・配信回数・直近配信の対象/送信数）のみを返す設計にした。
今後、開封率相当の指標が本当に必要になった場合は、リッチメニューのタップ計測やLIFF連携でのトラッキングを
別途設計する必要がある。営業資料・提案文言でこの点を混同しないよう申し送り。
